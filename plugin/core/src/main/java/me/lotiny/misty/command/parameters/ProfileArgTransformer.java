package me.lotiny.misty.command.parameters;
...
public class ProfileArgTransformer extends BukkitArgTransformer<Profile> {
    ...
    public Profile transform(CommandSender sender, String source) {
        Player player =
                (sender instanceof Player && source.equals("self")) ? (Player) sender : Bukkit.getPlayer(source);
        ...
        Optional<Profile> profileOpt =
                storageRegistry.getProfileStorage().findAsync("name", source).join();
