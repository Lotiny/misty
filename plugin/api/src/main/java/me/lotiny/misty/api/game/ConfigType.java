...
public enum ConfigType {
    GAME_TYPE(
            XMaterial.NAME_TAG, Integer.class, (game, val) -> game.setMaxTeamSize((int) val), GameSetting::getTeamSize),
    FINAL_HEAL(
            XMaterial.GLISTERING_MELON_SLICE,
            Integer.class,
            (game, val) -> game.setFinalHeal((int) val),
            GameSetting::getFinalHeal),
    GRACE_PERIOD(
            XMaterial.DIAMOND_SWORD,
            Integer.class,
            (game, val) -> game.setGracePeriod((int) val),
            GameSetting::getGracePeriod),
    BORDER_SIZE(
            XMaterial.BEDROCK, Integer.class, (game, val) -> game.setBorderSize((int) val), GameSetting::getBorderSize),
    FIRST_SHRINK(
            XMaterial.COBBLESTONE_WALL,
            Integer.class,
            (game, val) -> game.setFirstShrink((int) val),
            GameSetting::getFirstShrink),
    ...
    NETHER_TIME(
            ...
            Integer.class,
            (game, val) -> game.setNetherTime((int) val),
            GameSetting::getNetherTime),
    ...
    GOD_APPLE(
            ...
            Boolean.class,
            (game, val) -> game.setGodApple((boolean) val),
            GameSetting::isGodApple),
    LAST_BORDER_FLAT(
            XMaterial.GRASS_BLOCK,
            Boolean.class,
            (game, val) -> game.setLastBorderFlat((boolean) val),
            GameSetting::isLastBorderFlat),
    LATE_SCATTER(
            XMaterial.CLOCK,
            Boolean.class,
            (game, val) -> game.setLateScatter((boolean) val),
            GameSetting::isLateScatter),
    PEARL_DAMAGE(
            XMaterial.ENDER_PEARL,
            Boolean.class,
            (game, val) -> game.setPearlDamage((boolean) val),
            GameSetting::isPearlDamage),
    CHAT_BEFORE_PVP(
            XMaterial.OAK_SIGN,
            Boolean.class,
            (game, val) -> game.setChatBeforePvp((boolean) val),
            GameSetting::isChatBeforePvp),
    ...
    STRENGTH_1(
            XMaterial.POTION, Boolean.class, (game, val) -> game.setStrength1((boolean) val), GameSetting::isStrength1),
    STRENGTH_2(
            XMaterial.POTION, Boolean.class, (game, val) -> game.setStrength2((boolean) val), GameSetting::isStrength2),
    POISON(XMaterial.POTION, Boolean.class, (game, val) -> game.setPoison((boolean) val), GameSetting::isPoison),
    INVISIBLE(
            XMaterial.POTION, Boolean.class, (game, val) -> game.setInvisible((boolean) val), GameSetting::isInvisible);
    ...
    public void apply(GameSetting setting, Object value) {
        if (!expectedType.isInstance(value)) {
            throw new IllegalArgumentException("Expected " + expectedType.getSimpleName() + " but got "
